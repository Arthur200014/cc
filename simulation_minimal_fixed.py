...
total_neurons = len(thalamus_E_TCR) + len(thalamus_I_nRT) + len(L23_E) + len(L23_I) + len(L4_E) + len(L4_I) + len(L5_E) + len(L5_I) + len(L6_E) + len(L6_I)
...
print("✅ Симуляция завершена! График сохранён как simulation_result.png")

# Сохраняем результаты в текстовый файл
with open("results.txt", "w") as f:
    f.write(f"Всего нейронов: {total_neurons}\n")
    f.write(f"Время симуляции: {h.tstop} мс\n")
    f.write("Симуляция завершена успешно\n")
